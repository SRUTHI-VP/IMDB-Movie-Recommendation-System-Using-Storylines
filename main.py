...
# Load CSV
def load_data(csv_file="imdb_movies_cleaned(2).csv"):
    df = pd.read_csv(csv_file)
    df.dropna(subset=["Movie_Title", "Storyline"], inplace=True)
...
def recommend_movies(input_storyline, df, tfidf, tfidf_matrix, top_n=5):
    ...
    results = df.iloc[top_indices][["Movie_Title", "Storyline"]].copy()
...
def load_and_prepare_data():
    df = load_data("imdb_movies_cleaned(2).csv")
            ...
            for _, row in recommendations.iterrows():
                st.markdown(f"""
                    <div class='movie-box'>
                        ...
                        <p style='color: #333; line-height: 1.6;'>{row['Storyline']}</p>
                    </div>
                """, unsafe_allow_html=True)
